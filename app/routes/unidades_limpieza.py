...
@router.post("/", response_model=schemas_unidades.UnidadLimpiezaResponse)
def crear_unidad(unidad: schemas_unidades.UnidadLimpiezaCreate, db: Session = Depends(get_db)):
...
def obtener_unidades(skip: int = 0, limit: int = 100, db: Session = Depends(get_db)):
...
def obtener_unidad_por_id(unidad_id: int, db: Session = Depends(get_db)):
...
def actualizar_unidad(unidad_id: int, unidad: schemas_unidades.UnidadLimpiezaUpdate, db: Session = Depends(get_db)):
